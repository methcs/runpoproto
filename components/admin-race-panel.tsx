...
export default function AdminRacePanel() {
  ...
  const fetchRaces = async () => {
    try {
      ...
      if (!response.ok) {
        // Log server error and keep races as empty array to avoid runtime issues
        console.error('Error fetching races (server):', data)
        setRaces([])
        return
      }
      ...
      console.error('Error fetching races (network):', error)
      setRaces([])
    }
  }
